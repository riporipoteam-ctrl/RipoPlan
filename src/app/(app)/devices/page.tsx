    ...
    <>
      ...
      <div className="flex-1 space-y-3 px-4 py-4">
        <div className="rounded-2xl border border-[var(--border)] bg-gradient-to-br from-nebula-50 to-transparent p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-[var(--muted)]">{ctx.workspace.name} · shared</div>
            </div>
        ...
        <p className="px-1 text-xs text-[var(--muted)]">
          Agents run their tools on a shared cloud device. For long-running or stateful workloads,
          provision a dedicated container (e.g. E2B / Firecracker) and register it here — the
          architecture is wired for additional local or cloud devices.
        </p>

        <div className="flex items-center gap-3 rounded-2xl border border-dashed border-[var(--border)] p-4 text-sm text-[var(--muted)]">
          <Monitor size={18} />
          Local device — pair a machine to let agents act on your computer (coming soon).
        </div>
      </div>
    </>
  );
